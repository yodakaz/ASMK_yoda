...
def train(demo_train, training, validation, model, globals):
    ...
    for epoch in range(training['epochs']):
        ##################################################################################
        #start_epoch = 0
        #tempname = os.path.join("/home/yoda/Jupyterlab_3/content/how/how_temp","temp.pth")
        if os.path.exists("/home/yoda/Jupyterlab_3/content/how/how_temp/temp.pth"):
            tempname = "/home/yoda/Jupyterlab_3/content/how/how_temp/temp.pth"
            checkpoint = torch.load(tempname)
            epoch = checkpoint['epoch']
            net.meta = checkpoint['meta']
            net.load_state_dict(checkpoint['state_dict'])
            optimizer.load_state_dict(checkpoint['optimizer'])
            scheduler = checkpoint['scheduler']
            criterion = checkpoint['criterion']
            train_loader = checkpoint['train_loader']
            train_loss = checkpoint['losslogger']
            #validation.best_score[1] = checkpoint['best_score']
            validation.scores = checkpoint['scores']
            model = checkpoint['net_params']
            
            print("#######check loaded content######")
            print("scores : ", validation.scores)
        ...
        ###########################################################################################
        state = {
            'epoch': epoch1, 'meta': net.meta, 'state_dict': net.state_dict(), 'losslogger': train_loss,
            'optimizer' : optimizer.state_dict(), 'scheduler': scheduler, 'criterion': criterion,
            'train_loader': train_loader, 'best_score': validation.best_score[1],
            'scores': validation.scores, 'net_params': model, '_version': 'how/2020',
        }
        tempname = os.path.join("/home/yoda/Jupyterlab_3/content/how/how_temp","temp.pth")
        torch.save(state, tempname)
        print("#######check saved content######")
        print("scores : ", validation.scores)
        print("net_params : ", model)
        print("losslogger : ", train_loss)
        print("scheduler : ", scheduler)
        print("######saved after %d#######"%epoch1)
        ###########################################################################################
...
class Validation:
    ...
    def validate(self, net, epoch):
        ...
        for name, frequency in self.frequencies.items():
            print("!! show frequency !!",frequency)############################################
            print("!! change frequency==22 !!")############################################
            frequency = 22 #######################################################
